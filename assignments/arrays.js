...
const specificCar = function (arr1) {
    let carInfo = "";
  ...
  console.log(`Car 33 is a ${specificCar(inventory).car_year} ${specificCar(inventory).car_make} ${specificCar(inventory).car_model} `);

// STRETCH TASK VERSION ===============

let specificCarStretch = inventory.find(year => year.id === 33);
console.log(`Car 33 is a ${specificCarStretch.car_year} ${specificCarStretch.car_make} ${specificCarStretch.car_model} `);
...
const sortedModels = function(arr2) {
    let models = [];
    for(let i = 0; i < arr2.length; i++ ) {
      models.push(arr2[i].car_model);
      models.sort();
    }
    return models;
  }
  let carModels = sortedModels(inventory);
  console.log(carModels);

// STRETCH TASK VERSION ===============

let carModelsStretch = inventory.map(car => car.car_model).sort();
...
const arrCarYears = function(arr3) {
    const allYears = [];
    for(let i = 0; i < arr3.length; i++ ) {
       allYears.push(arr3[i].car_year);
    }
    return allYears;
  }
  
  let carYears = arrCarYears(inventory);
  console.log(carYears);

// STRETCH TASK VERSION ===============

let carYearsStretch = inventory.map(car => car.car_year);
...
const olderCarYears = function(arr4) {
    const olderYears = [];
      for(let i = 0; i < arr4.length; i++) {
        if(arr4[i] < 2000) {
            olderYears.push(arr4[i]);
           }
      }
      return olderYears;
    }
    
    let oldCars = olderCarYears(carYears);
    console.log(oldCars.length);

// STRETCH TASK VERSION ===============

let oldCarsStretch = carYears.filter(year => year < 2000).length;
console.log(oldCarsStretch);
...
const mokeSpecificCar = function(arr5) {
    const make = [];
      for(let i = 0; i < arr5.length; i++) {
        if(arr5[i].car_make === 'BMW' || arr5[i].car_make === 'Audi') {
            make.push(arr5[i]);
           }
      }
      return make;
    ...
    let BMWAndAudi = mokeSpecificCar(inventory);
    console.log(JSON.stringify(BMWAndAudi));

// STRETCH TASK VERSION ===============
let BMWAndAudiStretch = inventory.filter(make => make.car_make === 'BMW' || make.car_make === 'Audi');
